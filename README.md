...
- **효과가 적용되는 시점이 프레임 안에서 다르다.** 내장 러너는 `MotionPump`가 도는
  ...
  순서가 어긋날 수 있다.
- **`Delay` 노드는 이 백엔드를 거치지 않는다.** `DelayNode`는 순수 코어에 있어
  트윈 러너를 아예 모르고 `MotionHandle.FromTimer`를 직접 쓴다. 즉 그래프 안의
  `Delay`는 백엔드를 바꿔도 언제나 스코프의 델타로 돈다. 러너의 `onEased == null`
  분기는 인터페이스 계약을 지키기 위한 것이고 현재 그 경로를 타는 노드는 없다.
